...
export function splitIntoChunks(text: string): string[] {
  ...
  if (text.length > MAX_SUPPORTED_CHARS) {
    throw new Error(`This document is too long for bounded browser analysis. Please split it into files smaller than ${MAX_SUPPORTED_CHARS.toLocaleString()} characters.`);
  }
...
export function buildSynthesisInput(label: string, analyses: string[]): string {
  return analyses
    .map((analysis, index) => `--- ${label} section ${index + 1} ---\n${analysis.slice(0, 3500)}`)
    .join('\n\n');
}
