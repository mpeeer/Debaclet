...
export default function App() {
  ...
  const handleSubmit = async (_text: string, file: File | null) => {
    ...
    try {
      ...
      // No active browser generation, or the previous generation already ended.
    }
    ...
    try {
      if (cachedConfig.provider === 'webllm') {
        ...
        if (text.length > MAX_SUPPORTED_CHARS) throw new Error(`This document is too long for browser analysis. Please split it into files smaller than ${MAX_SUPPORTED_CHARS.toLocaleString()} characters.`);
        ...
        for (let i = 0; i < chunks.length; i += 1) {
          ...
          setLoadStep(2);
          setLoadProgress(chunks.length > 1 ? `Analyzing section ${i + 1} of ${chunks.length}…` : 'Waking up the AI...');
          const debaterResponse = await queryBrowserAI(cachedConfig.model, DEBATER_PROMPT, section, (msg, pct) => {
            if (requestId !== requestIdRef.current) return;
            if (msg.includes('already loaded')) setLoadProgress('Model ready');
            else if (chunks.length === 1) setLoadProgress(`${msg} (${pct}%)`);
          });
          if (requestId !== requestIdRef.current) return;
        ...
        if (chunks.length > 1) {
          setLoadStep(3);
          setLoadProgress('Synthesizing the complete document…');
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      // The engine may already have completed or have nothing to interrupt.
    } finally {
  ...
  const handleReset = () => { setState('idle'); setResult(null); setError(null); setCurrentFile(''); setDraft({ text: '', file: null }); setLoadProgress(''); setCompareMode(false); setCompareIds(new Set()); setEditingId(null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
                  ...
                  <div className="mt-10 animate-fade-in">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { setHistory([]); void clearHistory(); }} className="text-[11px] font-medium px-2 py-1 rounded-lg border border-surface-border hover:border-red-500/30 hover:text-red-400 transition-colors" style={{ color: 'rgb(var(--text-muted))' }}>Clear</button>
                      </div>
                    </div>
            ...
            {state === 'loading' && (
              <div className="flex flex-col items-center justify-center py-16 animate-fade-in">
                ...
                <h2 className="text-xl font-serif italic font-semibold mb-1.5" style={{ color: 'rgb(var(--text))' }}>
                  ...
                  {loadStep === 2 && 'Analyzing your argument…'}
                  {loadStep === 3 && 'Synthesizing the complete analysis…'}
                ...
                <p className="text-sm max-w-md text-center leading-relaxed" style={{ color: 'rgb(var(--text-muted))' }}>
                  {loadStep === 0 && 'Extracting the text from your document to prepare it for analysis.'}
                  {loadStep === 1 && 'First-time downloads can take a minute. The model stays cached for next time.'}
                  {loadStep === 2 && 'Reviewing each section for counterarguments and logical structure.'}
                  {loadStep === 3 && 'Combining the section analyses into one coherent result.'}
                  {loadStep === 4 && 'Almost done — putting everything together.'}
                </p>
                <button
                  type="button"
                  onClick={handleCancel}
                  className="mt-6 inline-flex items-center gap-2 rounded-xl border border-surface-border px-4 py-2 text-xs font-semibold transition-colors hover:border-red-400/50 hover:text-red-300"
                  style={{ color: 'rgb(var(--text-secondary))' }}
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
                  {isCancelling ? 'Stopping…' : 'Cancel analysis'}
                </button>
              </div>
            )}
